...
@IDE: pycharm
@file: PostionEmbedding.py
@time: 13/9/20 15:47
@desc:
'''
from keras import backend as K
from keras.layers import Embedding, Layer, Lambda
import numpy as np


def pos_encoding(time, n_waves, d_model=100):
    encodings = np.zeros(shape=(2 * n_waves, time))
    positions = np.arange(time, dtype='float32')
...
def pos_encoding_keras(x):
    time, n_waves, d_model = x[0], x[1], x[2]
    encodings = K.zeros(shape=(2 * n_waves, time))
    positions = K.arange(time, dtype="float32")
    for i in range(K.get_value(n_waves)):
        encodings[2 * i] = K.sin(positions / 10. ** (2. * i / d_model))
        encodings[2 * i + 1] = K.cos(positions / 10. ** (2. * i / d_model))
    return K.transpose(encodings)
